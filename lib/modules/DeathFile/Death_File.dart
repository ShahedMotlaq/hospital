...
import 'package:project2two/modules/DeathFile/Death_FileCubit.dart';
import 'package:project2two/modules/DeathFile/Death_FileState.dart';
...
class _DeathFileFormState extends State<DeathFileForm> {
  final TextEditingController identityStatusController = TextEditingController();
  ...
  final TextEditingController reasonLastHourController = TextEditingController();
  final TextEditingController liverMortiseLocationController = TextEditingController();
  final TextEditingController liverMortiseImproveController = TextEditingController();
  final TextEditingController liverMortiseColorController = TextEditingController();
  final TextEditingController rigorMortiseLocationController = TextEditingController();
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (BuildContext context) => DeathFileCubit(),
      child: BlocConsumer<DeathFileCubit, DeathFilestate>(
                    ...
                    children: [
                        ...
                        onTap: () async {
                          ...
                          if (pickedDate != null && pickedDate != deathDate) {
                            setState(() {
                              ...
                              deathDateController.text = DateFormat('yyyy-MM-dd').format(deathDate!);
                            });
                          }
                        },
                        child: AbsorbPointer(
                      ...
                      GestureDetector(
                        onTap: () async {
                          ...
                          if (pickedDate != null) {
                            setState(() {
                              fileDateController.text = DateFormat('yyyy-MM-dd').format(pickedDate);
                            });
                          }
                        },
                        child: AbsorbPointer(
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(labelText: 'Liver Mortise Location'),
                      ),
                      TextField(
                        controller: liverMortiseImproveController,
                        decoration: InputDecoration(labelText: 'Liver Mortise Improve'),
                      ),
                      TextField(
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(labelText: 'Rigor Mortise Location'),
                      ),
                      TextField(
                      ...
                      SizedBox(height: 16),
                      ConditionalBuilder(
                        condition: state is! DeathFileLoadingState,
                          ...
                          child: MaterialButton(
                            onPressed: () async {
                              if (formKey.currentState!.validate()) {
                                final deathFileData = {
                                  'deathFile': {
                                    'identityStatus': identityStatusController.text,
                                    'temperature': temperatureController.text,
                                    'deathLocation': deathLocationController.text,
                                    'deathDate': deathDateController.text,
                                    'deathHour': deathHourController.text,
                                  ...
                                  'deathReason': [
                                    {
                                      'lastMinute': lastMinuteController.text,
                                      'lastDay': lastDayController.text,
                                      'lastYear': lastYearController.text,
                                      'reasonLastHour': reasonLastHourController.text,
                                      'anatomy': anatomy,
                                      'autopsy': autopsy,
                                    },
                                  ],
                                  ...
                                  'liverMortise': {
                                    'location': liverMortiseLocationController.text,
                                    'improve': liverMortiseImproveController.text,
                                    'color': liverMortiseColorController.text,
                                    'removed': liverMortiseRemoved,
                                  },
                                  'rigorMortise': {
                                    'location': rigorMortiseLocationController.text,
                                  },
                                  'dehydration': dehydrationController.text,
                                  'lateSigns': lateSignsController.text,
                                };

                                DeathFileCubit.get(context).submitDeathFile(deathFileData: deathFileData);
                              }
                            },
                            color: Colors.blue,
